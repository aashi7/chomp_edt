...
class CHOMP
{
	protected:		
		...
		Matrix Dx_;         // Signed Distance Field
	
		// gradient descent 
		Matrix AA_;			// metric
		Matrix BB_;
		Matrix grid_;        // For computing Euclidean Distance Transform
	...
	public:
		CHOMP(double dt_input, double eta_input, double lambda_input, size_t nq_input, size_t cdim_input, size_t numIt_, double gain, double gamma_input);
		void initCHOMP(void); 			//Initializes gradient descent vectors and matrices
		void generatePath(Vector  &xi, Vector &ti);	//optimize path using with numIt_ iterations
		...
		double chompIteration(Vector  &xi, Vector &ti);	//performs a single gradient descent iteration
		...
		void initStackedPath(Vector  &qs, Vector &qe, double ts, double te);//initializes path with qs and qe, starting and ending point, where xi_ is stacked to qs point
		void setPath(Vector  &qs, Vector &qe, double ts, double tq, Vector &xi); // sets an aribitrary value for xi_, qs_, qe_
		void getPath(Vector &xi, Vector &ti); // gets xi_ 	
		void makeGrid(double xmin, double ymin, double xmax, double ymax);
};
...
{
   public:
   	CHOMP_SE2(double dt_input, double eta_input, double lambda_input, size_t nq_input, size_t numIt_, double gain, double gamma_input);
