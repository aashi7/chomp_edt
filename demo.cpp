...
Vector timeTrajectory;              // the time (t_1, t_2, ... t_n)
...
double ts;            // the start time
double te;            // the end time
...
//////////////////////////////////////////////////
// CHOMP
CHOMP chomp(1.0, 100.0, 1.0, nq, cdim, 1000, 5.0, 1.0);
...
gamma = 1.0 (trade-off between time and position smoothness)
*/
...
// Set two obstacles
static handle_s rep1 (55.0, 50.0, 5.0, 0.0, 0.0, 1.0, 0.2);
static handle_s rep2 (50.0, 55.0, 5.0, 0.0, 0.5, 1.0, 0.2);
...
public:
  ...
  void update (Vector const & position, double const time)
  {
    if (position.size() != 2) {
...
double const Robot::radius_ (0.05); // 0.5 previously
...
{	
  ...
  {
    robots[ii].update (trajectory.block (ii * cdim, 0, cdim, 1), timeTrajectory(ii));
  }
}
...
static void cb_jumble ()
{
  for (size_t ii (0); ii < xidim; ++ii) 
  {
   // trajectory[ii] = double (rand()) / (0.1 * numeric_limits<int>::max()) - 5.0;
    trajectory[ii] = double ((rand() % 10) + 50.0)  ;
  }

  for (size_t tt(0); tt < nq; ++tt)
  {
    timeTrajectory[tt] = double (ts + (tt+1)*(te-ts)/(nq+1));
  }

  //cout << trajectory << endl;
  //cout << timeTrajectory << endl;
  //update new initial path
  chomp.setPath(qs, qe, ts, te, trajectory);
  update_robots();
}
...
static void cb_idle ()
{
  ...
  //////////////////////////////////////////////////
  // One CHOMP iteration  
  chomp.setObstacles(obs); //update obstacles    // updating cost in each iteration by updating obstacles  
  //chomp.chompIteration(trajectory); //update path  // doing one iteration at once
  chomp.generatePath(trajectory, timeTrajectory);

 // cout << "Generated Path " << trajectory << endl;
...
static void cb_draw ()
{
  ...
  gfx::set_view (0.0, 0.0, 150.0, 150.0);
  chomp.makeGrid(0.0, 0.0, 150.0, 150.0);
...
int main()
{
  ...
  chomp.initStackedPath(qs, qe, ts, te);  
  chomp.getPath(trajectory, timeTrajectory);
  update_robots();   
  state = PAUSE;
